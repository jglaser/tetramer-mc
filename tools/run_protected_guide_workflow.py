#!/usr/bin/env python3
"""Run one frozen protected-guide validation, audits, then first-pass classification in order."""
...
NAME = 'run_protected_guide_workflow.py'
SCHEMA = 'protected-guide-validation-workflow-v1'
...
def verify_sources(campaign, expected):
    ...
    require({NAME, 'run_protected_guide_validation.py', 'analyze_protected_guide_validation.py'} <= set(sources),
...
def load_archived(campaign, expected):
    ...
    runner = importlib.import_module('run_protected_guide_validation')
    analyzer = importlib.import_module('analyze_protected_guide_validation')
    ...
    require(runner.preflight(campaign) == protocol, 'Validation preflight protocol differs')
...
def run(campaign, expected):
    ...
    state = dict(schema=SCHEMA, complete=False, phase='claimed', protocol_sha256=expected,
        ...
        workflow_sha256=protocol['python_sources'][NAME], analysis_workers=16)
    ...
    try:
        ...
        require(physical['complete'] is True and physical['phase'] == 'complete',
            'Physical validation and audits did not complete')
        ...
        result = analyzer.analyze(campaign, campaign / 'comparison', workers=16)

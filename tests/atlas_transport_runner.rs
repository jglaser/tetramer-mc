//! Actual runner control, reverse-density replay and restart for the fixed atlas.
use anyhow::Result;
...
#[test]
fn zero_update_matches_frozen_and_active_transport_replays_and_resumes() -> Result<()> {
    let root = std::env::temp_dir().join(format!("tetramer-atlas-runner-{}", std::process::id()));
    if root.exists() {
    ...
    cfg["atlas_transport"] = json!({"mean_gain":0.,"covariance_gain":0.,"weight_gain":0.,
    ...
    for (a, b) in frozen.iter().zip(&zero) {
        assert_eq!(
            a["poses"], b["poses"],
            "zero updates must preserve each physical bit"
        );
    ...
    for (a, b) in old_moves.iter().zip(new_moves) {
        for key in [
            ...
            assert_eq!(a[key], b[key], "zero control mismatch for {key}");
        }
        assert_eq!(
    ...
    run(options(&root, "active.json", "full", 48, None))?;
    run(options(&root, "active.json", "part", 19, None))?;
    run(options(&root, "active.json", "resume", 48, Some("part")))?;
            ...
            kind => {
                ...
                if row["hard_valid"] == true {
                    ...
                    let ratio = if kind == "global" {
                        ...
                        let forward = engine.model(&engine.fit(&poses)?, &state)?;
                        let reverse = engine.model(&engine.fit(&next)?, &state)?;
    ...
    broken["atlas_state"] = Value::Null;
    fs::write(root.join("part/checkpoint.json"), broken.to_string())?;
    assert!(run(options(&root, "active.json", "broken", 48, Some("part"))).is_err());
    fs::remove_dir_all(root)?;
    Ok(())
}

...
import run_protected_guide_workflow as workflow
...
def analyze(root,out,workers):
    assert workers==16 and json.loads((root/'status.json').read_text())['complete']
    out.mkdir()
...
class WorkflowTests(unittest.TestCase):
    def setUp(self):
        ...
        (common / 'run_protected_guide_validation.py').write_text(RUNNER)
        (common / 'analyze_protected_guide_validation.py').write_text(ANALYZER)
        ...
        old_modules = {name: sys.modules.pop(name, None) for name in ('run_protected_guide_validation', 'analyze_protected_guide_validation')}
        def restore():
    ...
    def test_checksum_and_source_changes_fail_before_claim(self):
        ...
        (self.root / 'common/analyze_protected_guide_validation.py').write_text('changed')

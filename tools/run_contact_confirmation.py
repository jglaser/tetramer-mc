#!/usr/bin/env python3
"""Freeze and run a separate five-arm full-R4 contact-weight confirmation.

Twenty independent populations compare proposal width, count, defensive mass and
Poisson intensity. The fixed 80-component guide retains all old components and
adds all eight fresh training populations in each of three predeclared strata.
No physical samples from the old training campaigns are rerun or pooled.
"""
...
RUNNER_NAME = 'run_contact_confirmation.py'
SCHEMA = 'contact-confirmation-controller-v1'
PACKAGE_SCHEMA = 'refined-contact-bank-preparation-v1'
...
# This allocation is fixed in source before freeze; independent of observed output.
MAIN_DRAWS = 131072
ARMS = [dict(id=name, guide=guide, covariance_factor=width, samples=draws, alpha=alpha,
             lambda_ratio=intensity, component_count=80)
        for name, guide, width, draws, alpha, intensity in (
            ('bank', 'bank', 1., MAIN_DRAWS, .5, 128.),
            ('wide', 'wide', 4., MAIN_DRAWS, .5, 128.),
            ('small', 'bank', 1., 32768, .5, 128.),
            ('alpha02', 'alpha02', 1., MAIN_DRAWS, .2, 128.),
            ('intensity256', 'bank', 1., MAIN_DRAWS, .5, 256.))]
SEEDS = [133101010+1009*i for i in range(20)]
TOTAL = sum(4*a['samples'] for a in ARMS)
COMPONENTS = 80
RETAINED_GUIDE_SHA256 = '5bc58a35d8a87c8677f73d997860595790cb7fe0c875414ce17a59dc8184002c'
PINS = {
...
COMPONENT_GROUPS = [
    dict(source_group='conditional-ray', class_name='native', count=24, component_weight=1/192, full_probability_at_alpha_half=.0625),
    dict(source_group='conditional-ray', class_name='no-entry', count=24, component_weight=1/96, full_probability_at_alpha_half=.125),
    dict(source_group='R5-reference', class_name='native', count=8, component_weight=1/64, full_probability_at_alpha_half=.0625),
    *[dict(source_group='fresh-pilot', class_name=name, count=8, component_weight=1/48, full_probability_at_alpha_half=1/12)
      for name in ('old_R5_intersection_native','remaining_R4_native','contact_no_native_entry')],
]
...
def validate_package(package):
    ...
    required = {'plan.json', 'config.json', 'region.json', 'shape.json', 'native-definition.json',
                'guide-bank.json', 'guide-wide.json', 'guide-alpha02.json', 'native-region/definition.json', 'selection.json', 'old-r5-region.json', 'native-partition-definition.json', 'retained-guide.json'}
    ...
    require(sha(package/'old-r5-region.json') == plan['reference_region_sha256'] and
            sha(package/'native-partition-definition.json') == plan['supplemental_definition_sha256'], 'Native partition binding changed')
    partition = read(package/'native-partition-definition.json')
    require(partition['schema'] == 'contact-confirmation-native-partition-v1' and
            partition['reference_region_sha256'] == plan['reference_region_sha256'] and
            partition['region_sha256'] == plan['region_sha256'] and
            partition['native_definition_sha256'] == plan['native_definition_sha256'], 'Native partition target changed')
    guides = {}
    for arm in ARMS:
        name = arm['guide']; path = package/f'guide-{name}.json'; guide = read(path)
        ...
        require(parsed.count == COMPONENTS and parsed.alpha == arm['alpha'], 'Wrong defensive guide allocation')
        guides[name] = guide
    require(sha(package/'retained-guide.json') == RETAINED_GUIDE_SHA256, 'Retained old guide changed')
    retained = read(package/'retained-guide.json')
    require(len(retained['gaussian_components']) == 56 and retained['defensive_uniform_shell_probability'] == .5
            and retained['region_sha256'] == plan['region_sha256'], 'Retained guide law changed')
    for old, current in zip(retained['gaussian_components'], guides['bank']['gaussian_components'][:56]):
        require(old['mean'] == current['mean'] and old['covariance'] == current['covariance'] and
                current['weight'] == .5*old['weight'], 'Retained component geometry/weight changed')
    ...
    require(guides['alpha02']['gaussian_components'] == guides['bank']['gaussian_components'], 'Defensive control changed Gaussian law')
    require(sha(package/'selection.json') == plan['selection_sha256'], 'Training selection changed')
    ...
    require(all(a['source_group'] == ('conditional-ray' if i < 48 else 'R5-reference' if i < 56 else 'fresh-pilot') for i, a in enumerate(anchors)),
    ...
    require(len({(a['arm'], a['population_id']) for a in anchors[56:]}) == 8 and all(
        {a['class'] for a in anchors[56:] if (a['arm'], a['population_id']) == population}
        == {'old_R5_intersection_native', 'remaining_R4_native', 'contact_no_native_entry'}
        for population in {(a['arm'], a['population_id']) for a in anchors[56:]}), 'Fresh strata slots changed')
...
def freeze(out, binary, bundle_path, package, source_root=None):
    ...
    bundle, rust = verify_bundle(binary, bundle_path, Path(source_root).resolve() if source_root else ROOT)
    python = local_dependencies([Path(__file__), ROOT/'tools/analyze_latent_region.py', ROOT/'tools/analyze_contact_confirmation.py'])
    ...
    for arm_index, spec in enumerate(ARMS):
        ...
        for name, path in [('source-bundle.json', bundle_path), ('latent-region-normalizer', binary),
                           ...
                           ('region.json', package/'region.json'), ('importance-guide.json', package/f"guide-{arm['guide']}.json")]:
    ...
    protocol = dict(schema=SCHEMA, arms=arms, jobs=jobs, total_unconditional_draws=TOTAL, maximum_physical_workers=8,
        ...
        decision_arms=['bank','wide','alpha02','intensity256'],
        component_groups=COMPONENT_GROUPS,
        component_group_scope='Full probabilities are tabulated at alpha=0.5; multiply by 2*(1-alpha) for the alpha02 arm. Gaussian conditional weights are identical across arms.',
        ...
        comparisons=[['bank', name] for name in ('wide','small','alpha02','intensity256')],
        ...
        scope='Fixed fresh confirmation, separate from all training evidence. No retries, adaptive stopping, or pooling arms. All 20 physical populations finish and validate before five archived audits. All-attempt weights and native partition checks precede any whole-vessel or assembly inference.')
...
def validate(out):
    ...
    closure = local_dependencies([common/RUNNER_NAME, common/'analyze_latent_region.py', common/'analyze_contact_confirmation.py'])
    ...
    require(protocol['decision_arms'] == ['bank','wide','alpha02','intensity256'] and
            protocol['comparisons'] == [['bank', name] for name in ('wide','small','alpha02','intensity256')], 'Decision allocation changed')
    for key, name in [('reference_region','old-r5-region.json'), ('supplemental_definition','native-partition-definition.json')]:
        require(protocol[key] == 'common/reference-package/'+name and
                sha(inside(out, protocol[key])) == protocol[key+'_sha256'] == plan[key+'_sha256'], 'Native partition path/hash changed')
    all_jobs = []
    require(len(protocol['arms']) == len(ARMS), 'Incomplete arm allocation')
    for index, (arm, expected) in enumerate(zip(protocol['arms'], ARMS)):
        ...
        require(manifest['schema'] == CAMPAIGN_SCHEMA and len(manifest['jobs']) == 4 and manifest['workers'] == 4
                and manifest['physical_activity'] == .035 and manifest['lambda_ratio'] == arm['lambda_ratio'] and manifest['cloud_replicates'] == 2,
        ...
        require(sha(archive/'importance-guide.json') == manifest['importance_guide_sha256'] == plan['guide_sha256'][arm['guide']], 'Guide changed')
        ...
        for i, job in enumerate(manifest['jobs']):
            ...
            all_jobs.append(job)
    require(all_jobs == protocol['jobs'] and len(all_jobs) == 20 and sum(j['samples'] for j in all_jobs) == TOTAL, 'Incomplete allocation')
    return protocol
...
def preflight(out):
    ...
    require(not (out/'status.json').exists(), 'Confirmation already launched; no retries')
...
def verify_output(out, protocol, job):
    ...
    expected = dict(schema=POPULATION_SCHEMA, samples=job['samples'], seed=job['seed'], cloud_replicates=2,
        activity=.035, lambda_ratio=arm['lambda_ratio'], importance_uniform_probability=arm['alpha'], importance_component_count=COMPONENTS,
...
def run(out):
    ...
    state = dict(schema='contact-confirmation-status-v1', complete=False, phase='physical', started=time.time(),
...
def validate_completed(out):
    ...
    require(status['schema'] == 'contact-confirmation-status-v1' and status['complete'] is True
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source-root', type=Path, help='Exact reviewed source tree associated with the reused executable')
    args = parser.parse_args()
    if args.action == 'freeze':
        if any(x is None for x in (args.binary, args.source_bundle, args.package, args.source_root)):
            parser.error('freeze requires binary, source-bundle and package')
        result = freeze(args.out, args.binary, args.source_bundle, args.package, args.source_root)
    else:

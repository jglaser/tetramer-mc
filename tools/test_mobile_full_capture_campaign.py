...
import numpy as np
import prepare_mobile_full_capture_campaign as preparation
import run_mobile_full_capture_campaign as controller
...
class FullCaptureControllerTests(unittest.TestCase):
    def setUp(self):
        self.temporary=tempfile.TemporaryDirectory(prefix='full-capture-controller-test-');self.addCleanup(self.temporary.cleanup)
        ...
        for arm_index,(arm,epsilon) in enumerate(preparation.ARMS.items()):
            folder=self.campaign/arm;archive=folder/'provenance'
            for name in ('runs','logs','provenance'):(folder/name).mkdir(parents=True)
            for name in ('basin-normalizer','config.json','model.json','shape.json',preparation.ANALYZER_NAME):
                (archive/name).write_text('not an executable; mocks only')
            jobs=preparation.jobs_for(folder,arm_index,epsilon)
            ...
            self.protocol['campaigns'].append(dict(arm=arm,path=str(folder),manifest_sha256=preparation.sha(folder/'manifest.json')))
    ...
    def test_all_physics_drains_and_validates_before_single_audit_per_arm(self):
    ...
    def test_physical_output_validation_failure_forbids_any_audit(self):
        with (patch.object(controller.subprocess,'Popen',side_effect=self.start),
              patch.object(controller,'verify_population',side_effect=ValueError('wrong model')),
              patch.object(controller.subprocess,'run') as audits):
            with self.assertRaisesRegex(ValueError,'wrong model'):controller.run(self.campaign,self.journal)
        self.assertTrue(all(c.waits==1 for c in self.children));audits.assert_not_called()
    ...
    def test_reservation_collision_does_not_launch_or_overwrite(self):
        other=self.campaign/'epsilon-0p5/status.json';preparation.write(other,dict(owner='other'))
...
class FullCaptureDesignTests(unittest.TestCase):
    def test_exact_eight_fresh_streams_and_both_anchor_cli(self):
        jobs=[j for index,(arm,epsilon) in enumerate(preparation.ARMS.items())
            for j in preparation.jobs_for(Path('/unused')/arm,index,epsilon)]
        self.assertEqual([j['seed'] for j in jobs],[122101010+1009*j for j in range(8)])
        for index,job in enumerate(jobs):
            ...
            self.assertEqual(argv[argv.index('--covariance-scale')+1],'1')
            self.assertEqual(float(argv[argv.index('--uniform-probability')+1]),.1 if index<4 else .5)
            self.assertNotIn('--proposal-anchor-index',argv)
            self.assertIsNone(job['proposal_anchor_index'])
    ...
    def test_relocation_changes_only_shape_path(self):
        original=dict(shape='/source/shape.json',fixed_poses=[{'position':[1.,2.,3.]}],capture_radius=170.,reservoir_density=.035,
            depletant_radius=1.5,metadata={'frozen':'scaffold'},seed=19,uniform_probability=.1)
    ...
    def test_exact_physics_and_whole_reciprocal_atlas_fail_closed(self):
        config=preparation.read(preparation.REFERENCE/'config.json')
        model=preparation.read(preparation.MODEL)
        preparation.validate_physics(config,model)
        for key,value in [('capture_radius',18.),('poisson_lambda_ratio',1.),('depletant_radius',1.4),
                ('reservoir_density',.034),('fixed_poses',[]),('target_region',{'rho':12})]:
            bad=copy.deepcopy(config);bad[key]=value
            with self.assertRaises((ValueError,AssertionError)):preparation.validate_physics(bad,model)
        bad=copy.deepcopy(model);bad['reciprocal_components'][0]=False
        with self.assertRaises(ValueError):preparation.validate_physics(config,bad)

    def test_exhaustive_partition_keeps_seam_and_boundaries(self):
        cfg=preparation.read(preparation.REFERENCE/'config.json')
        part=preparation.partition_metadata(cfg)
        self.assertEqual(len(part['primary_partition']),3)
        self.assertEqual(part['chart_anchor'],cfg['fixed_poses'][0])
        self.assertIn('exact competitor Cayley seam',part['primary_partition'][2]['criterion'])
        self.assertIn('q=1 belongs to native',part['boundary_rules'])
        self.assertIn('rho_comp=12 belongs to competitor ball',part['boundary_rules'])
        self.assertIn('unconditional',part['row_accounting'])
        self.assertFalse(part['native_instantaneous_entry']['hysteresis'])

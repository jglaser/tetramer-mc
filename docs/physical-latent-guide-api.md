...
The vessel loop and independent audit are now implemented and have passed the synthetic reference checks described in [full-vessel latent mixture](full-vessel-latent-mixture.md). The following contract describes the implemented integration:
...
The independent [Python physical-density helper](../tools/physical_latent_guide.py) now passes seven mathematical tests. It reconstructs eight Rust-generated world-pose records in two charts, including exterior and near-seam cases; the largest scaled coordinate/density error is 4.60e-16. The [validation archive](../runs/physical-latent-guide-math-validation-20260922/python-validation.json) retains tests, fixture bindings and the independent audit. These initial checks tested coordinates and densities. Subsequent integrated audits independently reconstructed 768 fresh sphere-reference draws, including valid exterior-R4 contributions; see the [integrated validation](../runs/full-vessel-reference-dispatch-20260922/validation.json). The separate baseline auditor and complete contact/pocket partition now support an [inert matched vessel preparation](full-vessel-comparison-preparation.md). Protein execution remains gated on regional convergence. No full-vessel protein comparison, coverage conclusion or assembly inference has been produced by this API work.

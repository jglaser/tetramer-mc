#!/usr/bin/env python3
"""Matched frozen-atlas control with progressively enabled parameter transport.

Reuse the exact dispersed starts, MC seeds, physical parameters, and native-
informed atlas of a completed campaign. This isolates changing the proposal
parameters from replacing the information supplied by that atlas.
"""
...
ARMS = ('frozen', 'means', 'covariance', 'weights')
...
def main():
    ...
    previous_jobs = [j for j in previous['jobs'] if j.get('mode', j.get('variant')) == 'frozen']
    if not previous_jobs:
        parser.error('Source campaign needs frozen-arm configurations')
    ...
    if args.atlas_json:
        inputs['atlas-options.json'] = args.atlas_json.resolve()
    ...
    base_options = json.loads(args.atlas_json.read_text()) if args.atlas_json else {}
    jobs = []
    for old_job in previous_jobs:
        ...
        for arm in ARMS:
            ...
            cfg['metadata'].update(
                ...
                training_feedback=arm != 'frozen',
                feedback_scope='Current pair geometry only; no accumulated training archive')
            if arm != 'frozen':
                cfg['atlas_transport'] = dict(
                    mean_gain=.25, covariance_gain=.25 if arm in ('covariance', 'weights') else 0.,
                    weight_gain=.25 if arm == 'weights' else 0.,
                    mean_noise=.1, covariance_noise=.1 if arm in ('covariance', 'weights') else 0.,
                    weight_noise=.1 if arm == 'weights' else 0.,
                    assignment_cutoff=6., residual_clip=3., shrinkage=8.,
                    refresh_probability=1., initialization='reference')
                cfg['atlas_transport'].update(base_options)
    ...
    manifest = dict(schema=1, created_unix_time=time.time(), jobs=jobs,
                    ...
                    scope='Reused independent starts and paired MC seeds; all arms retain the same native-informed atlas, rigid shapes, depletion, local and collective kernels. Fixed component count. This is a controlled assembly-accessibility pilot, not an equilibrium or speedup estimate.')

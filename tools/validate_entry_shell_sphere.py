#!/usr/bin/env python3
"""End-to-end sphere/AO and Haar quadrature control of the entry-shell guide.
...
BINARY=ROOT/'target/entry-shell-review/release/latent-region-normalizer'
BUNDLE=ROOT/'target/entry-shell-review/release/build/tetramer-mc-356a8e0b913327fe/out/source-bundle.json'
RADIUS=2.4
...
def validate(out):
    ...
    write(out/'validation-plan.json',dict(samples_per_run=16384,seeds=[130100011,130100012],activities=[0.,2.],
    ...
    for index,z in enumerate((0.,2.)):
        ...
        guide=dict(schema='defensive-entry-shell-guide-v1',region_sha256=sha(prov/'region.json'),defensive_uniform_shell_probability=.2,
            entries=[dict(weight=1/3,moving_member=[0.,0.,0.],target_world_member=[0.,0.,0.],inner_radius=.6,outer_radius=hi)for hi in (.7,1.4,2.4)])
        ...
        job=dict(id='r00',seed=130100011+index,samples=16384,directory=str(root/'runs/r00'))
        manifest=dict(schema='importance-latent-region-campaign-v2',jobs=[job],workers=1,lambda_ratio=64.,cloud_replicates=2,
        ...
        sampling=analysis['importance_sampling'];require(sampling['shell_rejected']>0 and sampling['branch_counts']['entry-shell']>0,'Missing exterior or guided probes')
    ...
    record=dict(schema='entry-shell-sphere-crosslanguage-v1',complete=True,results=results,binary_sha256=sha(BINARY),

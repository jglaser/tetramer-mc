...
## Periodic coordinate metadata compatibility

The current runtime uses the four definitions in [the periodic-compatible asset archive](../runs/finite-assembly-observer-periodic-20260924/manifest.json). In the analysis command above, set `--definition` to `runs/finite-assembly-observer-periodic-20260924/definitions/N12-spherical.json`, or the matching size/boundary file. The earlier archive and receipts remain unchanged and require their matching archived observer implementation.

The updated validator recognizes exactly two coordinate descriptions for periodic runs: the legacy text emitted by older executables and the corrected canonical `[0,L)` description. Spherical runs retain their original strict description. Input, geometry, boundary and provenance checks remain in force. Each of the four new definition files differs from its predecessor only in `source_sha256`; physical identities, region rules, shape, patches, native definitions and cached graph bytes are unchanged.

The [compatibility receipt](../runs/finite-assembly-observer-periodic-validation-20260924/validation.json) records 28 direct tests and 46 downstream tests, both previously completed without repetition for asset preparation. The new freeze authenticates 47 files, including 16 source files. Preparation preserved all 48 cached starting-state graphs and made zero native-classifier calls, interbody geometry replays or physical draws. Manifest SHA-256 is `d2c2bd6dc0571df919361e41090351523ec7566c2ff0629824443d323300d871`.

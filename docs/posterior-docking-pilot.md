...
The next useful bounded check is a frozen regional normalizer around this cohort, retaining the already validated physical target and fresh confirmation draws. A longer rate benchmark should wait for that coverage check. A later fixed-budget option is 16 paired runs of25,000 cycles total (eight per method), with the same four starts repeated under independent seeds: 400,000 cycles overall, approximately 2.1 summed CPU hours at pilot cost. This would nominally yield about30 c=0.9 core returns if the pilot rate persisted, but neither that expectation nor statistical stationarity is guaranteed. **No extension has been launched.**
